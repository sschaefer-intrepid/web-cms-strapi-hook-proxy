  ...
  for (const pod of pods) {
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      ...
      try {
        ...
        console.error(`Attempt ${attempt} failed for ${pod.metadata.name}:`, error.message);
...
app.post('/webhook', checkSecretKey, async (req, res) => {
  ...
  if(strapiEvent === undefined || strapiEvent !== 'entry.create' && strapiEvent !== 'entry.update' && strapiEvent !== 'entry.delete' && strapiEvent !== 'entry.publish' && strapiEvent !== 'entry.unpublish')
  {
  ...
  if (pods.length === 0) {
    console.log(`No pods found in namespace: ${webAocNamespaceName} for deployment ${webAocDeploymentName}`);
    return;
  }

  console.log(`Found ${pods.length} pods for deployment ${webAocDeploymentName} in namespace: ${webAocNamespaceName}`);
